...
# run container
docker run -d -v $apppath:/var/www -p 80:80 --link mapbiomas-server-db:dbhost -h mapbiomas-server-web --name mapbiomas-server-web terras/mapbiomas-server-web ./start.sh
