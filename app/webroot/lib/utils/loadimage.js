...
ImageLoad.prototype.newImage = function(imageURI, callback_load, callback_progress) {
    ...
    this.loadImage(queue);

    this.requests.push(queue);

}

ImageLoad.prototype.loadImage = function(queue) {

    var self = this;

    queue.request.open("GET", queue.uri, true);

    queue.request.onreadystatechange = function(evt) {
        if (evt.currentTarget.status == 400 || evt.currentTarget.status == 429) {
            setTimeout(function(){              
              self.loadImage(queue);              
            },2000);
        }
